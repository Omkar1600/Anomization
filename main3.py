...
# Function to create triggers for non-admin users
def create_triggers(connection, table_name, selected_user):
    # Trigger for INSERT operation
    trigger_insert_query = f"""
    CREATE TRIGGER {selected_user}_insert_trigger
    AFTER INSERT ON {table_name}
    FOR EACH ROW
    BEGIN
        INSERT INTO {selected_user}_view VALUES (NEW.*);
    END
    """
    
    # Trigger for UPDATE operation
    trigger_update_query = f"""
    CREATE TRIGGER {selected_user}_update_trigger
    AFTER UPDATE ON {table_name}
    FOR EACH ROW
    BEGIN
        UPDATE {selected_user}_view
        SET {', '.join([f'{column}=NEW.{column}' for column in column_names])}
        WHERE <condition>;
    END
    """
    
    # Trigger for DELETE operation
    trigger_delete_query = f"""
    CREATE TRIGGER {selected_user}_delete_trigger
    AFTER DELETE ON {table_name}
    FOR EACH ROW
    BEGIN
        DELETE FROM {selected_user}_view WHERE <condition>;
    END
    """
    
    try:
        cursor = connection.cursor()
        cursor.execute(trigger_insert_query)
        cursor.execute(trigger_update_query)
        cursor.execute(trigger_delete_query)
        connection.commit()
        st.success("Triggers created successfully.")
    except Error as e:
        st.error(f"Error creating triggers: {e}")
    ...
    if host and database and username and password:
        ...
        if connection:
            ...
            if is_admin(connection, username):
                ...
                # Check if user is non-admin and create triggers accordingly
                if not is_admin(connection, username):
                    create_triggers(connection, selected_table, username)

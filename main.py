...
# Function to update user data using a stored procedure
def update_user_data(connection, user_id, new_data):
    try:
        cursor = connection.cursor()
        cursor.callproc('update_user_data', (user_id, new_data))
        connection.commit()
        st.success("User data updated successfully.")
    except Error as e:
        st.error(f"Error updating user data: {e}")

# Function to create a stored procedure to update user data
def create_update_user_data_procedure(connection):
    try:
        cursor = connection.cursor()
        cursor.execute("""
            CREATE PROCEDURE update_user_data(
                IN username VARCHAR(255),
                IN new_data VARCHAR(255)
            )
            BEGIN
                -- Update the user data in the personal_info table based on the username
                UPDATE personal_info SET data = new_data WHERE username = username;
            END
        """)
        st.success("Stored procedure 'update_user_data' created successfully.")
    except Error as e:
